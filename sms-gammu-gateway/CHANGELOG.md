...
* record the SIM7670G as verified through the add-on ([f4ea352](https://github.com/HomeOps/HASS-SMS-Gammu-Gateway/commit/f4ea352244467b3f2674bddf4cbfeede6f161e07))
* record the SIM7670G as verified through the add-on ([dc068d6](https://github.com/HomeOps/HASS-SMS-Gammu-Gateway/commit/dc068d68c27b13ad9cc301f4707fdd30c7cd3b35))
...
> This entry was regenerated by hand. release-please had no tag to bound its
> first run, so it swept the whole repository history and listed every upstream
> feature commit here as if it were new. Subsequent releases are bounded by the
> `sms-gammu-gateway-v1.8.0` tag and do not have this problem.

### Bug Fixes

* map ttyACM4-7 and enable uart. The device list stopped at ttyACM3, so on a host
  with several USB serial devices the modem lands outside it after a
  re-enumeration and the add-on fails Init with ERR_DEVICEOPENERROR until the
  modem is physically replugged.

### Build and packaging

* compile libGammu from source with the SMS prompt fix from
  [gammu/gammu#1177](https://github.com/gammu/gammu/pull/1177). The packaged
  library only recognises an SMS prompt of `"> "`, so GSM_SendSMS stalls until
  commtimeout on modems that emit a bare `">"` (SIMCom SIM7670G among them).
* the add-on is renamed to **SMS Gammu Gateway (HomeOps)** with slug
  `sms_gammu_gateway_homeops` and its own MQTT topic prefix and device id, so it
  can be installed alongside the upstream add-on.

### Documentation

* credit upstream authors and record third-party licences.
